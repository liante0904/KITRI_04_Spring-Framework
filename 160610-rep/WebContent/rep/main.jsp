...
<script type="text/javascript">
	window.onload=function(){
		var param = "type=list";
		sendRequest("${pageContext.request.contextPath}/RepController", 
	...
	function listResult(){
		if(httpRequest.readyState==4){
			if(httpRequest.status==200){
				...
				var repListD = document.getElementById("repList");			
				for(i=0;i<repList.length;i++){
					repListD.appendChild(makeRepDiv(repList[i]));	
				}
			}
		}
	}	
	...
	function makeRepDiv(rep){
		...
		var html = "이름: " + rep.name + " 내용: " + rep.content + " <input type='button' value='수정' onclick='showUpdateForm(" + rep.num + ")'> <input type='button' value='삭제' onclick='delRep(" + rep.num + ")'><br />"
	...
	function delRep(num){
		...
		sendRequest("${pageContext.request.contextPath}/RepController",	params, delRepResult, "POST");
	}
	function delRepResult(){
		if(httpRequest.readyState==4){
			if(httpRequest.status==200){
				...
				var repList = document.getElementById("repList");
				var repDiv = document.getElementById("rep_"+o.num);
	...
	function showUpdateForm(num){
		...
		sendRequest("${pageContext.request.contextPath}/RepController", param, getRepResult, "POST");
	}
	...
	function editRep(){
		var params = "type=editRep&num=" +document.updateForm.u_num.value +
		"&name=" +document.updateForm.u_name.value + 
		"&content="	+document.updateForm.u_content.value;
		sendRequest("${pageContext.request.contextPath}/RepController", params, editRepResult, "POST");
		hideUpdateForm();
	}
	...
	function editRepResult(){
		...
		if(httpRequest.readyState==4){
			if(httpRequest.status==200){
				...
				var repDiv = makeRepDiv(rep);
				var repList = document.getElementById("repList");
				var oldDiv = document.getElementById("rep_"+rep.num);
	...
	<div id="updateFormDiv">
		<form name="updateForm" style="display: none">
			...
			<table border='1'>
				...
				<tr>
					<th>content</th>
					<td><input type="text" name="u_content"></td>
				</tr>
				<tr>
					<td colspan="2"><input type="button" value="수정" onclick="editRep()"> 
					<input type="button" value="취소" 	onclick="hideUpdateForm()"></td>
				</tr>
			</table>
